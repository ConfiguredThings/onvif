 ...
 */
const parseSOAPString = function(xml, callback, statusCode, statusMessage) {
	...
	try {
		xml2js.parseString(
			...
			function(err, result) {
				if (statusCode != 200) {
					callback(new Error(`HTTP Error ${statusCode} ${statusMessage}`), null, xml, statusCode);
				} else if (!result || !result['envelope'] || !result['envelope']['body']) {
