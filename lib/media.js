...
module.exports = function(Cam) {
	...
	/** Added by CT */
	Cam.prototype.getVideoSourceConfigurationOptions = function(options, callback) {
		if (callback === undefined) {
			callback = options;
			if (this.videoSourceConfigurations && this.videoSourceConfigurations[0]) {
				options = { configurationToken: this.videoSourceConfigurations[0].$.token };
			} else {
				return callback(new Error('No video source configuration token is present!'));
			}
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetVideoSourceConfigurationOptions xmlns="${xmlns}">` +
					(
			...
			if (callback) {
				callback.call(this, err, err ? null : linerase(data[0].getVideoSourceConfigurationOptionsResponse[0].options), xml);
			}
		}.bind(this));
	};
	...
	Cam.prototype.getAudioEncoderConfigurationOptions = function(options, callback) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetAudioEncoderConfigurationOptions xmlns="${xmlns}">` +
					(
						(options.configurationToken)
							? '<ConfigurationToken>' + options.configurationToken + '</ConfigurationToken>'
							: ""
	...
	/** Added by CT */
	Cam.prototype.getAudioSourceConfigurationOptions = function(options, callback) {
		if (callback === undefined) {
			callback = options;
			if (this.audioSourceConfigurations && this.audioSourceConfigurations[0]) {
				options = { configurationToken: this.audioSourceConfigurations[0].$.token };
			} else {
				return callback(new Error('No audio source configuration token is present!'));
			}
		} else if (typeof options == "string") { // For backward compatibility
			options = { configurationToken: options };
		//} else if (!(options && (options.configurationToken || options.profileToken))) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetAudioSourceConfigurationOptions xmlns="${xmlns}">` +
					(
						(options.configurationToken)
							? '<ConfigurationToken>' + options.configurationToken + '</ConfigurationToken>'
							: ""
			...
			if (callback) {
				callback.call(this, err, err ? null : linerase(data[0].getAudioSourceConfigurationOptionsResponse[0].options), xml);
			}
		}.bind(this));
	};
	...
	Cam.prototype.getAudioOutputConfigurations = function(callback) {
		...
		/** Added by CT */
	Cam.prototype.getAudioOutputConfigurationOptions = function(options, callback) {
		if (callback === undefined) {
			callback = options;
			if (this.audioOutputConfigurations && this.audioOutputConfigurations[0]) {
				options = { configurationToken: this.audioOutputConfigurations[0].$.token };
			} else {
				return callback(new Error('No audio source configuration token is present!'));
			}
		} else if (typeof options == "string") { // For backward compatibility
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetAudioOutputConfigurationOptions xmlns="${xmlns}">` +
					(
						(options.configurationToken)
							? '<ConfigurationToken>' + options.configurationToken + '</ConfigurationToken>'
							: ""
			...
			if (callback) {
				callback.call(this, err, err ? null : linerase(data[0].getAudioOutputConfigurationOptionsResponse[0].options), xml);
			}
		}.bind(this));
	};
	...
	Cam.prototype.getSnapshotUri = function(options, callback) {
		...
		if (this.media2Support) {
			...
			this._request({
				...
				if (callback) {
					if (this.media == 'ver20') {
						callback.call(this, err, err ? null : linerase(data).getSnapshotUriResponse.uri, xml);
					} else {
						// Slight difference in Media1 and Media2 reply XML
						// Generate a reply that looks like a Media1 reply for existing library users
   ...
   */
	...
	/**
	 * Get all existing metadata configurations of a device
	 * @param {Cam~MetadataConfigurationsCallback} callback
	 */
	Cam.prototype.getMetadataConfigurations = function(callback) {
		...
		this._request({
			service: service
			, body: this._envelopeHeader() +
			`<GetMetadataConfigurations xmlns="${xmlns}"/>` +
			this._envelopeFooter()
		}, function(err, data, xml) {
			if (!err) {
				this.metadataConfigurations = data[0].getMetadataConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
			}
			if (callback) {
				callback.call(this, err, this.metadataConfigurations, xml);
			}
		}.bind(this));
	};

	Cam.prototype.getMetadataConfigurationOptions = function(options, callback) {
		if (callback === undefined) {
			callback = options;
			if (this.metadataConfigurations && this.metadataConfigurations[0]) {
				options = { configurationToken: this.metadataConfigurations[0].$.token };
			} else {
				return callback(new Error('No metadata configuration token is present!'));
			}
		} else if (typeof options == "string") { // For backward compatibility
			options = { configurationToken: options };
		}
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetMetadataConfigurationOptions xmlns="${xmlns}">` +
					(
						(options.configurationToken)
							? '<ConfigurationToken>' + options.configurationToken + '</ConfigurationToken>'
							: ""
			...
			if (callback) {
				callback.call(this, err, err ? null : linerase(data[0].getMetadataConfigurationOptionsResponse[0].options), xml);
			}
		}.bind(this));
	};
	 ...
	 * Added by CT
	 *
	 * Note these are only supported by ver10/media
	 */
	Cam.prototype.getCompatibleAudioEncoderConfigurations = function(options, callback) {

		if (typeof options == "string") { // For backward compatibility
			options = { profileToken: options };
		}
		const service = 'media';
		const xmlns = 'http://www.onvif.org/ver10/media/wsdl';
		this._request({
			service: service
			, body: this._envelopeHeader() +
			(
				(options) ?
					(
						`<GetCompatibleAudioEncoderConfigurations xmlns="${xmlns}">` +
					(
						(options.profileToken)
							? '<ProfileToken>' + options.profileToken + '</ProfileToken>'
							: ""
			...
			if (callback) {
				let configs = data[0].getCompatibleAudioEncoderConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
				callback.call(this, err, err ? null : configs, xml);
			}
		}.bind(this));
	};

	
	Cam.prototype.getCompatibleAudioOutputConfigurations = function(options, callback) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetCompatibleAudioOutputConfigurations xmlns="${xmlns}">` +
					(
						(options.profileToken)
							? '<ProfileToken>' + options.profileToken + '</ProfileToken>'
							: ""
			...
			if (callback) {
				let configs = data[0].getCompatibleAudioOutputConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
				callback.call(this, err, err ? null : configs, xml);
			}
	...
	Cam.prototype.getCompatibleAudioSourceConfigurations = function(options, callback) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetCompatibleAudioSourceConfigurations xmlns="${xmlns}">` +
					(
						(options.profileToken)
							? '<ProfileToken>' + options.profileToken + '</ProfileToken>'
							: ""
			...
			if (callback) {
				let configs = data[0].getCompatibleAudioSourceConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
				callback.call(this, err, err ? null : configs, xml);
			}
	...
	Cam.prototype.getCompatibleMetadataConfigurations = function(options, callback) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetCompatibleMetadataConfigurations xmlns="${xmlns}">` +
					(
						(options.profileToken)
							? '<ProfileToken>' + options.profileToken + '</ProfileToken>'
							: ""
			...
			if (callback) {
				let configs = data[0].getCompatibleMetadataConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
				callback.call(this, err, err ? null : configs, xml);
			}
	...
	Cam.prototype.getCompatibleVideoSourceConfigurations = function(options, callback) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetCompatibleVideoSourceConfigurations xmlns="${xmlns}">` +
					(
						(options.profileToken)
							? '<ProfileToken>' + options.profileToken + '</ProfileToken>'
							: ""
			...
			if (callback) {
				let configs = data[0].getCompatibleVideoSourceConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
				callback.call(this, err, err ? null : configs, xml);
			}
	...
	Cam.prototype.getCompatibleVideoEncoderConfigurations = function(options, callback) {
		...
		this._request({
			...
			(
				(options) ?
					(
						`<GetCompatibleVideoEncoderConfigurations xmlns="${xmlns}">` +
					(
						(options.profileToken)
							? '<ProfileToken>' + options.profileToken + '</ProfileToken>'
							: ""
			...
			if (callback) {
				let configs = data[0].getCompatibleVideoEncoderConfigurationsResponse[0].configurations.map(function(config) {
					return linerase(config);
				});
				callback.call(this, err, err ? null : configs, xml);
			}
