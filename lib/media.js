   ...
   */
	...
	Cam.prototype.getCompatibleAudioEncoderConfigurations = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				if (!err) {
					configs = data[0].getCompatibleAudioEncoderConfigurationsResponse[0].configurations.map(function(config) {
						return linerase(config);
					});
				}
	...
	Cam.prototype.getCompatibleAudioOutputConfigurations = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				let configs = null;
				if (!err) {
					configs = data[0].getCompatibleAudioOutputConfigurationsResponse[0].configurations.map(function(config) {
						return linerase(config);
					});
				}
				callback.call(this, err, err ? null : configs, xml);
	...
	Cam.prototype.getCompatibleAudioSourceConfigurations = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				if (!err) {
					configs = data[0].getCompatibleAudioSourceConfigurationsResponse[0].configurations.map(function(config) {
						return linerase(config);
					});
				}
				callback.call(this, err, err ? null : configs, xml);
	...
	Cam.prototype.getCompatibleMetadataConfigurations = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				let configs = null
				if (!err) {
					configs = data[0].getCompatibleMetadataConfigurationsResponse[0].configurations.map(function(config) {
						return linerase(config);
					});
				}
				callback.call(this, err, err ? null : configs, xml);
	...
	Cam.prototype.getCompatibleVideoSourceConfigurations = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				let configs = null;
				if (!err) {
					configs = data[0].getCompatibleVideoSourceConfigurationsResponse[0].configurations.map(function(config) {
						return linerase(config);
					});
				}
				callback.call(this, err, err ? null : configs, xml);
	...
	Cam.prototype.getCompatibleVideoEncoderConfigurations = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				let configs = null;
				if (!err) {
					configs = data[0].getCompatibleVideoEncoderConfigurationsResponse[0].configurations.map(function(config) {
						return linerase(config);
					});
				}
				callback.call(this, err, err ? null : configs, xml);
