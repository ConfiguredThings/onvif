 ...
 * @property {string} [media] force media to always be "ver10" or "ver 20" (where applicable)
 */
...
var Cam = function(options, callback) {
	...
	switch (this.media) {
		case null:
			// Default mode from upstream - false until
			// ver20/media detected in result of getServices,
			// and then some responses mapped back to ver10
			this.media2Support = false;
			break;

		case 'ver10':
			// only call media/ver10 even the device supports ver20
			this.media2Support = false;
			break;

		case 'ver20':
			// use media/ver20 apis and return ver20 objects
			this.media2Support = true;
			break;

		default:
			throw new Error(`Invalid media option ${this.media}`);
	}
...
Cam.prototype.connect = function(callback) {
	...
	this.getSystemDateAndTime(function(err, date, xml) {
		...
		this.getServices(true, function(err) {
			...
			if (this.media2Support && !(this.media == 'ver20')) {
...
Cam.prototype._requestPart2 = function(options, callback) {
	...
	const req = httpLib.request(reqOptions, function(res) {
		...
		res.on('end', function() {
			...
			parseSOAPString(xml, callback, statusCode, statusMessage); // xml and statusCode are passed in. Callback is the function called after the XML is parsed
		});
	});
...
Cam.prototype.getServices = function(includeCapability, callback) {
	...
	this._request({
		...
		if (!err) {
			...
			// If the media version hasn't been specified then
			// see if ver20 is supported
			if (this.media === null) {
